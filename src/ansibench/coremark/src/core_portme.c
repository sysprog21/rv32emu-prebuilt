...
/* Function: core_start_parallel
    ...
    Three implementations are provided, one using pthreads, one using fork and
   shared mem, and one using fork and sockets. Other implementations using MCAPI
   or other standards can easily be devised.
*/
...
{
    return (ee_u8) pthread_create(&(res->port.thread), NULL, iterate,
                                  (void *) res);
}
...
ee_u8 core_start_parallel(core_results *res)
{
    ...
    if (res->port.pid == 0) { /* benchmark child */
        ...
        if (-1 == res->port.sock) /* if socket failed to initialize, exit */ {
            ...
            int bytes_sent = sendto(res->port.sock, &(res->crc), buffer_length,
                                    0, (struct sockaddr *) &(res->port.sa),
    ...
    res->port.sock = socket(PF_INET, SOCK_DGRAM, IPPROTO_UDP);
    bound = bind(res->port.sock, (struct sockaddr *) &(res->port.sa),
...
ee_u8 core_stop_parallel(core_results *res)
{
    ...
    int recsize = recvfrom(res->port.sock, &(res->crc), 8, 0,
                           (struct sockaddr *) &(res->port.sa), &fromlen);
    if (recsize < 0) {
