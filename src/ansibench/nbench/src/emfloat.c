                           ...
                           ulong arraysize)
{
    ...
    */
    ...
    for (i = 0; i < arraysize;
         i++) { /*       LongToInternalFPF(randwc(50000L),&locFPF1); */
    ...
    */
    while (loops--) {
        for (i = 0; i < arraysize; i++)
            switch (jtable[i % 16]) {
            case 0: /* Add */
                AddSubInternalFPF(0, abase + i, bbase + i, cbase + i);
                break;
            case 1: /* Subtract */
                AddSubInternalFPF(1, abase + i, bbase + i, cbase + i);
                break;
            case 2: /* Multiply */
                MultiplyInternalFPF(abase + i, bbase + i, cbase + i);
                break;
            case 3: /* Divide */
                DivideInternalFPF(abase + i, bbase + i, cbase + i);
                break;
            }
#ifdef DEBUG
        {
...
void RoundInternalFPF(InternalFPF *ptr)
{
    ...
    if (ptr->type == IFPF_IS_NORMAL || ptr->type == IFPF_IS_SUBNORMAL) {
        ...
        if (ptr->type != IFPF_IS_ZERO) {
            ...
            ** Check for overflow
            */
            /*              Does not do anything as ptr->exp is a short and
               MAX_EXP=37268 if (ptr->exp > MAX_EXP)
                            {
        ...
        */
        ...
        if (locx.sign ^ locy.sign ^ operation) {
            ...
            */
            borrow = 0;
            for (i = (INTERNAL_FPF_PRECISION - 1); i >= 0; i--)
                Sub16Bits(&borrow, &z->mantissa[i], locx.mantissa[i],
                          locy.mantissa[i]);
            ...
            if (borrow) {
                ...
                */
                ...
                borrow = 0;
                for (i = (INTERNAL_FPF_PRECISION - 1); i >= 0; i--) {
                    Sub16Bits(&borrow, &z->mantissa[i], 0, z->mantissa[i]);
                }
            } else {
            ...
            carry = 0;
            for (i = (INTERNAL_FPF_PRECISION - 1); i >= 0; i--) {
                Add16Bits(&carry, &z->mantissa[i], locx.mantissa[i],
                          locy.mantissa[i]);
            }
            ...
            if (carry) {
                ...
                */
                ...
                for (j = (INTERNAL_FPF_PRECISION - 1); j >= 0; j--)
                    Add16Bits(&carry, &z->mantissa[j], z->mantissa[j],
                              x->mantissa[j]);
            } else {
                carry = 0;
            }
    ...
    */
    ...
    case NORMAL_NORMAL:
        ...
        */
        z->type = x->type;
        z->sign = x->sign ^ y->sign;
        z->exp = x->exp - y->exp + ((INTERNAL_FPF_PRECISION * 16 * 2));
        for (i = 0; i < INTERNAL_FPF_PRECISION; i++) {
            z->mantissa[i] = 0;
            extra_bits[i] = 0;
        }
        ...
        while ((z->mantissa[0] & 0x8000) == 0) {
            ...
            */
            carry = 0;
            for (j = (INTERNAL_FPF_PRECISION - 1); j >= 0; j--)
                Sub16Bits(&carry, &extra_bits[j], extra_bits[j],
                          y->mantissa[j]);
    ...
    for (i = 0; i < 6; i++)
        if (i == 1) { /* Emit decimal point */
            ...
            *dest++ = ('0' + carryaccum);
            ...
            carryaccum = 0;
            memcpy((void *) &IFPF10, (void *) &locFPFNum, sizeof(InternalFPF));
            ...
            for (j = 0; j < 9; j++) {
                ...
                for (k = (INTERNAL_FPF_PRECISION - 1); k >= 0; k--)
                    Add16Bits(&mycarry, &(IFPFComp.mantissa[k]),
                              locFPFNum.mantissa[k], IFPF10.mantissa[k]);
                carryaccum += mycarry ? 1 : 0;
                memcpy((void *) &locFPFNum, (void *) &IFPFComp,
                       sizeof(InternalFPF));
            }
        }
